...
def get_parser():
    ...
    parser.add_argument(
        '--config',
        default='./config/st_relation/nturgbd-cross-view/train_log_lr_2_4.yaml',
...
class Processor():
    ...
    def adjust_learning_rate_log_lr(self, epoch):
        start = self.arg.lr_milestones[0]
        stop = self.arg.lr_milestones[1]
        x = np.logspace(start, stop, num=arg.num_epoch)
        ratio = x[epoch] / (10 ** start)
        print("ratio: ", ratio)
        lr = self.arg.base_lr * ratio
        for param_group in self.optimizer.param_groups:
            param_group['lr'] = lr
        return lr
    ...
    def train(self, epoch, save_model=False):
        ...
        if self.arg.log_lr:
            lr = self.adjust_learning_rate_log_lr(epoch)
        else:
